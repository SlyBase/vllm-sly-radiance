...
| `patch_lmhead_int4_ct.py` | Same `radiance_lmhead_int4` hook for `CompressedTensorsConfig.get_quant_method` (0.2.4; moved behind the scheme lookup in 0.3.6): in the `ParallelLMHead` branch, before the lookup's result is used, so an `ignore`d head gets the int4 method instead of staying bf16 (RedHatAI/Qwen3.8-27B-INT4 ignores lm_head: 2.37 GiB bf16 per call instead of the int4 head). The DFlash drafter builds its head without a quant_config and shares the target head, so only the target head changes; `RADIANCE_LMHEAD_INT4` unset = stock. A head the checkpoint stores quantized (unsloth/Qwen3.8-27B-NVFP4: FP8 per-channel) gets `RadianceLMHeadInt4Over`, which lets the checkpoint's own `CompressedTensorsLinearMethod` load it (and `radiance_nvfp4` dequantize it to bf16) before the int4 pass; before 0.3.6 that head would have been loaded as unscaled e4m3 codes into a bf16 parameter. If the head is still fp8 after its own load step (`RADIANCE_NVFP4_MXFP4` off) it stays on the checkpoint's method, with a warning. Numerics: lm_head.weight (and embed_tokens) of RedHatAI/Qwen3.8-27B-INT4 are bit-identical to amd/Qwen3.8-27B-Quark-AWQ-MXFP4, so the `check_lmhead_int4.py` results carry over unchanged; G3 on 2026-09-17 with the hook active: GSM8K 0.84 (k=7 and k=4), needle 120k/258k correct |
...
| `patch_nvfp4_compile_key.py` | Adds `RADIANCE_NVFP4_*` to `envs.compile_factors()` (0.3.6): `patch_nvfp4_mxfp4.py` (top level, from ggz14) picks the scheme of every compressed-tensors linear and the lm_head at load from those knobs, so a flip on the same checkpoint changes the traced graph. Unset knobs add nothing (Quark/INT4 keys unchanged). Anchors on `patch_embed_int8` output |
...
| `patch_ar_knobs.py` | 0.3.5: `RADIANCE_AR_MAX_KB` / `RADIANCE_AR_QUANT_MIN_KB` / `RADIANCE_AR_QNT` / `RADIANCE_AR_QNB` as env knobs in `radiance_allreduce.py` (StillDeadcode's N-rank module; ggz14's `patch_ar_maxbytes.py` / `patch_ar_geometry.py` anchor on the older two-rank file). Defaults are the shipped values; only a TP=2 serve with the env set changes. The 48 MB size gate has to hold `tokens × hidden × 2` of the largest chunk, or every prefill all-reduce silently rides RCCL |
| `r4d/r4d_extras_rx10.patch` | 0.3.5: ggz14's `r4d_radiance_extras_rx10.patch` (against libr4d b9e42ab) rebased onto the pinned 5dc6302 and applied before `build.sh`. Kernel sources as-is; `build.sh`, `r4d.h`, `r4d_module.hip`, `r4d_registry.hip` merged by hand (additive conflicts next to the pin's N-rank all-reduce). Adds the narrow-state (bf16/fp16 SSM cache) GDN decode kernels, the lazy-snapshot GDN kernels, the fused GDN decode step, `R4D_ATTN_FP8` and the TP=2 all-reduce with fused epilogue; the 3-rank all-reduce is dropped (no binding in the N-rank module, TP=3 rides RCCL) |
