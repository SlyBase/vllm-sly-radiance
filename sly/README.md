...
| `patch_embed_int8.py` | Hooks `radiance_embed_int8` (embed_tokens as int8 rows, `RADIANCE_EMBED_INT8=1`, or int4 g128 with `RADIANCE_EMBED_BITS=4`; 1.18 / 1.74 GiB back for the KV cache) in front of the int4 lm_head block in `QuarkConfig.get_quant_method`; adds `RADIANCE_EMBED_*` to `envs.compile_factors()`. Anchors on `patch_lmhead_int4` and `patch_fused_norm_quant` output, runs last |
