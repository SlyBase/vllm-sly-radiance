...
| `patch_lmhead_int4_ct.py` | Same `radiance_lmhead_int4` hook for `CompressedTensorsConfig.get_quant_method`: first thing in the `ParallelLMHead` branch, before the scheme lookup whose `ignore` match would keep the head bf16 (RedHatAI/Qwen3.8-27B-INT4 ignores lm_head: 2.37 GiB bf16 per call instead of the int4 head). The DFlash drafter builds its head without a quant_config and shares the target head, so only the target head changes; `RADIANCE_LMHEAD_INT4` unset = stock |
